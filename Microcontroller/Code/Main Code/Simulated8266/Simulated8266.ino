...
SensorData sensorData(2);       // creating sensor data object to handle sensor data formatting, printing (for checking) and posting to server
...
float baseTemperature = random(20, 25);
float basePressure = random(80, 100);
...
void loop() {
  if (timeStatus() != timeNotSet) {
    ...
    if (timer1Minute.isTimerUp()) {
      ...
      sensorData.setTemperature(simulateTemperature());
      sensorData.setPressure(simulatePressure());
...
float simulateTemperature() {
  long swing = random(-100, 100);
  float delta = (float)swing / 100;
  baseTemperature += delta;
  return baseTemperature;
}

float simulatePressure() {
  float swing = random(-1000, 1000);
  float delta = (float) swing / 1000;
  basePressure += delta;
  return basePressure;
}
